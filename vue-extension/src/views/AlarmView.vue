  ...
  >
    <div class="flex items-center gap-2">
      ...
      <div class="flex-1 min-w-0">
        ...
        <p class="text-xs text-gray-500 line-clamp-1">{{ alarm.message }}</p>
      </div>
...
const removeAlarm = async (notificationId) => {
  try {
    ...
    console.error("알림 삭제 실패:", error.response.data.message);
  }
};
...
const acceptInvite = async (notificationId) => {
  try {
    const response = await api.post('/collections/shared/invite', {
    ...
    console.error("초대 수락 실패:", error.response?.data?.message || error.message);
  }
};
</script>
